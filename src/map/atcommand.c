 ...
 *------------------------------------------*/
void
jobchange_sub_display_jobs (const int fd)
{
...
/*==========================================
 * Expulsa todos os jogadores com nível de GM menor ou igual ao seu.
 *------------------------------------------*/
ACMD_FUNC(kickall)
{
	struct map_session_data* pl_sd;
...
ACMD_FUNC(kickat)
{
	...
	clif_displaymessage(fd, "Todos os jogadores usando autotrade foram expulsos."); // All players have been kicked!
...
ACMD_FUNC(autotrade)
{
	...
	if( map[sd->bl.m].flag.autotrade != battle_config.autotrade_mapflag ) {
		clif_displaymessage(fd, "O uso de autotrade não é permitido neste mapa.");
		return -1;
	}

	if( pc_isdead(sd) ) {
		clif_displaymessage(fd, "Você não pode usar autotrade estando morto.");
		return -1;
	}
	...
	if( battle_config.at_timeout ) {
		...
		if( !message || !*message ) {
			if( battle_config.at_only_timeout ) {
				clif_displaymessage(fd, "Digite @autotrade seguido do tempo em minutos.");
				return -1;
			} else {
		...
		if( timeout < 1 ) {
			clif_displaymessage(fd, "Digite um tempo inteiro, positivo e diferente de zero.");
			return -1;
		} else if( ( timeout > battle_config.at_max_timeout ) && battle_config.at_max_timeout ) {
			clif_displaymessage(fd, "Você digitou um valor muito alto.");
			return -1;
		}
