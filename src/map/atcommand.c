...
//Fun��o reloadnpc - Recarrega a toda a lista de NPCs salvas no emulador sem recarregar os NPCs para carregar apenas o NPC solicitado [Raizen]
ACMD_FUNC(reloadnpc)
{
	FILE *fp;
	...
	if( !sd->state.vending && !sd->state.buyingstore ) { //check if player is vending or buying
        ...
        if( battle_config.at_timeout ) {
				...
				if( timeout < 1 ) {
                        ...
                        clif_displaymessage(fd, msg_txt(1392)); // Voc� digitou um valor muito alto.
                        return -1;
                }
