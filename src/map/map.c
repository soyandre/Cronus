...
void do_abort(void)
{
	...
	{
		if (pc_db->size(pc_db))
			ShowFatalError("Servidor fechou sem conex„o ao char-server, %u personagens n„o foram salvos!\n", pc_db->size(pc_db));
		return;
	}
...
int do_init(int argc, char *argv[])
{
	...
	if (!map_ip_set || !char_ip_set) {
		...
		ShowInfo("Definindo %s como nosso endereÁo IP.\n", ip_str);
	...
	ShowStatus("Servidor est· '"CL_GREEN"pronto"CL_RESET"'. (Funcionando pela porta '"CL_WHITE"%d"CL_RESET"'.\n\n", map_port);
